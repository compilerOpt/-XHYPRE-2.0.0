...
/*--------------------------------------------------------------------------
 * hypre_HIGCSRMatrixMatvec
 *--------------------------------------------------------------------------*/

/* y[offset:end] = alpha*A[offset:end,:]*x + beta*b[offset:end] */
HYPRE_Int
hypre_HIGCSRMatrixMatvecOutOfPlace( HYPRE_Complex    alpha,
                                 hypre_CSRMatrix *A,
                                 hypre_Vector    *x,
                                 HYPRE_Complex    beta,
                                 hypre_Vector    *b,
...
#pragma omp parallel for private(i) HYPRE_SMP_SCHEDULE
            ...
            for (i = 0; i < num_rows*num_vectors; i++)
			{
				temp1 = two_prod (b_data[i] , temp);
			    y_data[i] = temp1.L + temp1.H;
			}
         }
      }
      else
...
#pragma omp parallel for private(i,j,jj,m,tempx) HYPRE_SMP_SCHEDULE
         ...
         for (i = 0; i < num_rownnz; i++)
         {
			...
			if ( num_vectors==1 )
            {
			   ...
			   {
				  temp1 = two_prod(A_data[jj], x_data[A_j[jj]]);
               ...
               for ( j=0; j<num_vectors; ++j )
               {
                  ...
                  for (jj = A_i[m]; jj < A_i[m+1]; jj++) 
				  {
					  temp1 = two_prod(A_data[jj], x_data[ j*vecstride_x + A_j[jj]*idxstride_x ]);
					  temps = two_sum(temps.H, temp1.H);
				      tempx = tempx + (temp1.L + temps.L) ;
				  }
                  tempx = tempx + temps.H;
...
HYPRE_Int
hypre_HIGCSRMatrixMatvec( HYPRE_Complex    alpha,
                       hypre_CSRMatrix *A,
                       hypre_Vector    *x,
                       HYPRE_Complex    beta,
                       hypre_Vector    *y     )
{
   return hypre_HIGCSRMatrixMatvecOutOfPlace(alpha, A, x, beta, y, y, 0);
}

/*--------------------------------------------------------------------------
 * hypre_HIGCSRMatrixMatvecT
 *
...
HYPRE_Int
hypre_HIGCSRMatrixMatvecT( HYPRE_Complex    alpha,
                        hypre_CSRMatrix *A,
                        hypre_Vector    *x,
                        HYPRE_Complex    beta,
                        hypre_Vector    *y     )
...
/*--------------------------------------------------------------------------
 * hypre_HIGCSRMatrixMatvec_FF
 *--------------------------------------------------------------------------*/

HYPRE_Int
hypre_HIGCSRMatrixMatvec_FF( HYPRE_Complex    alpha,
                          hypre_CSRMatrix *A,
                          hypre_Vector    *x,
                          HYPRE_Complex    beta,
                          hypre_Vector    *y,
